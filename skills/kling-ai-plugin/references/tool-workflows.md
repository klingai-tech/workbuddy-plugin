...
For standalone subject-library operations, motion-library browsing, or upload, use [asset workflows](asset-workflows.md); do not create a paid generation for an asset-only request.

1. Read `tools/list` from remote `kling-ai-plugin` at `https://kling.ai/mcp/plugin`. Before generation, call `who_am_i` and use only the target model's declared arguments, defaults, allowed values, and media inputs.
...
3. At each stage, select the tool and model before mapping media. Prefer accepted host references; when a local file has none, check conditional two-step upload in the asset workflow and stop if unavailable. Rebuild `arguments` and `inputs` from that model's live schema, run failure-prevention gates, and query credits immediately before submission.
